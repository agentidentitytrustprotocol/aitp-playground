...
| `revoke_tct` | `issuer`, `audience`, optional `via_cp`, optional `reason` | Walks the event log to find the most recent TCT `issuer` granted to `audience`, then POSTs its jti to `issuer`'s `/admin/revoke-tct`. When `via_cp: true`, also POSTs the jti to the CP's `/api/revocation/entries` and asks the audience to pull the updated list from `/.well-known/aitp-revocation-list` (the CP signs it; the playground does not yet verify the signature — `PENDING.md` P8) — see `intra-org/revocation-via-cp`. |
...
| `intra-org/revocation-via-cp@1.0.0` | RFC-AITP-0008 federation: revocation *data* propagates through the CP's `/.well-known/aitp-revocation-list` to an unrelated peer. Two caveats, both in the scenario's own summary: the snapshot's signature is not yet verified (`PENDING.md` P8), and the final 403 comes from the issuer's local deny-set, not the CP-derived one — so propagation is shown, enforcement-from-propagation is not. |
...
| `intra-org/tct-cache-perf@1.0.0` | TCT verification cache: repeated capability calls hit the SDK's `TctStore`; `tct_cache_stats` shows hits vs. misses. |
...
| `intra-org/tct-renewal@1.0.0` | RFC-AITP-0013 in-band TCT renewal: handshake → call → renew → call-with-new-jti. |
